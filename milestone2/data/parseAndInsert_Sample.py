...
def insert2BusinessTable():
    ...
    with open('.//yelp_dataset//yelp_business.JSON','r') as f:    #TODO: update path for the input file
        ...
        while line:
            ...
            sql_str = "INSERT INTO businessTable (business_id, name, address,state,city,zipcode,stars,reviewcount) " \
                      "VALUES ('" + cleanStr4SQL(data['business_id']) + "','" + cleanStr4SQL(data["name"]) + "','" + cleanStr4SQL(data["full_address"]) + "','" + \
                      cleanStr4SQL(data["state"]) + "','" + cleanStr4SQL(data["city"]) + "','" + cleanStr4SQL(data["full_address"][-5:]) + "'," + \
                      "," + str(data["stars"]) + "," + str(data["review_count"]) + ",0 ," + ");"
            try:
                cur.execute(sql_str)
            except:
